...
export default function Navbar() {
  ...
  return (
    <div className="w-full bg-white fixed top-0 z-50 flex justify-between items-center px-8 py-2 shadow-2xl rounded-b-full">
      <div>
      ...
      <nav className="hidden sm:flex gap-x-6 lg:gap-x-12">
        {navLinks.map((link) => (
          <Link
            key={link.title}
            to={link.path}
            className="font-medium hover:text-red-500 duration-200 hover:scale-125"
          >
            {link.title}
          </Link>
        ))}
